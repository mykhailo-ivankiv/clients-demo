        ...
        {substringEntriesIndices
          .reduce<[string[], number]>(
            ([result, offset], index) => {
              ...
              const [prefix, partWithSearchQueryAtStart] = splitAt(index - offset, last as string)
              const [query, suffix] = splitAt(
                searchQueries.find((searchQuery) =>
                  partWithSearchQueryAtStart.toLowerCase().startsWith(searchQuery.toLowerCase()),
                )?.length ?? 0,
          ...
          .map((str, i) => (i % 2 === 0 ? str : <mark key={i} className="bg-yellow-100" children={str} />))}
      </>
    )
  }
}
